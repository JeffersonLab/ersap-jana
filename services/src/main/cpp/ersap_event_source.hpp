...
public:
    ...
    ErsapEventSource(std::string res_name, JApplication* app) : JEventSource(std::move(res_name), app) {
        EnableFinishEvent();
    }
    ...
    void GetEvent(std::shared_ptr<JEvent> event) override {
        ...
        event->GetFactory<InputT>()->SetFactoryFlag(JFactory::JFactory_Flags_t::NOT_OBJECT_OWNER);
    ...
    void FinishEvent(JEvent& event) override {
        ...
        auto input = event.GetSingle<InputT>();
        auto output = event.GetSingle<OutputT>();
